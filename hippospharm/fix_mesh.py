...
def quadric_decimation_garland(resampled_mesh, target_vertices):
    ...
    print("INFO fixmesh.py:Garland quadric decimation: reducing vertices from %d to %d" % (positions.shape[0], target_vertices))
    if positions.shape[0] < target_vertices:
        # this will not work
    ...
    print("INFO fixmesh.py:Vertices after Garland quadric decimation: %d" % resampled_mesh.vertices.shape[0])
    return resampled_mesh
...
def mesh_fix(mesh: trimesh.Trimesh) -> trimesh.Trimesh:
    ...
    try:
        ...
        print("WARN fixmesh.py:pymeshfix import failed: %s. Skipping pymeshfix-based fixes." % e)
        return mesh
    ...
    print("INFO fixmesh.py:pymeshfix completed: resulting vertices=%d, faces=%d" %
...
def add_vertices_by_edge_split(mesh: trimesh.Trimesh, target_vertices: int) -> trimesh.Trimesh:
    ...
    if to_add <= 0 or to_add > 50:
        if to_add > 50:
            print("WARN fixmesh.py:Requested to add %d vertices (more than 50). Skipping edge-split insertion." % to_add)
        else:
            print("INFO fixmesh.py:No edge-split required: current vertices=%d target=%d" % (num_v_current, target_vertices))
        return mesh

    print("INFO fixmesh.py:Adding %d vertices to mesh by iterative edge split (current=%d, target=%d)" %
    ...
    for i in range(to_add):
        ...
        print("DEBUG fixmesh.py:Added vertex %d/%d (new total=%d)" % (i + 1, to_add, mesh.vertices.shape[0]))
    print("INFO fixmesh.py:Finished adding vertices: new vertex count=%d" % mesh.vertices.shape[0])
    return mesh
...
def fix_mesh(mesh_filename: str, target_vertices: int = 6890, remesh_bin=None, suffix='.obj',
    ...
    print("INFO fixmesh.py:Loaded original mesh: broken_faces=%s, vertices=%d" % (num_broken, num_vertices))

    if num_broken == 0 and abs(num_vertices - target_vertices) < tolerance_num_vertices:
        print("INFO fixmesh.py:Mesh has no holes and is within vertex tolerance (%d ± %d). Skipping remeshing." %
    ...
    if num_broken > 0:
        ...
        if use_mesh_fix:
            print("INFO fixmesh.py:Applying mesh_fix (pymeshfix) to original mesh before remeshing.")
            original_mesh = mesh_fix(original_mesh)
        broken_faces = trimesh.repair.broken_faces(original_mesh)
        print("INFO fixmesh.py:Broken faces after filling/fixing original mesh: %d" % len(broken_faces))
        if len(broken_faces) > 0:
            print("WARN fixmesh.py:Original mesh still has broken faces after pre-remeshing fixes.")
        ...
        print("INFO fixmesh.py:Exported pre-fixed mesh for remeshing: %s" % mesh_filename)
    ...
    try:
        ...
        print("INFO fixmesh.py:Executing remeshing binary: %s" % " ".join(cmd))
        result = subprocess.run(cmd, check=True, capture_output=True, text=True)
        print("DEBUG fixmesh.py:Remeshing stdout: %s" % result.stdout)
        print("DEBUG fixmesh.py:Remeshing stderr: %s" % result.stderr)
    except subprocess.CalledProcessError as e:
        print("Remeshing binary failed: returncode=%s, stderr=%s" % (getattr(e, 'returncode', None), getattr(e, 'stderr', None)))
        raise

    mesh = trimesh.load_mesh(manifold_output_filename)
    broken_faces = trimesh.repair.broken_faces(mesh)
    print("INFO fixmesh.py:Manifold output: broken_faces=%d, vertices=%d" % (len(broken_faces), mesh.vertices.shape[0]))

    if len(broken_faces) > 0:
        print("INFO fixmesh.py:Filling holes in manifold output.")
        # trimesh.repair.fill_holes(mesh)
        if use_mesh_fix:
            print("INFO fixmesh.py:Applying mesh_fix (pymeshfix) to manifold output.")
            mesh = mesh_fix(mesh)
        broken_faces = trimesh.repair.broken_faces(mesh)
        print("INFO fixmesh.py:Broken faces after filling/fixing: %d" % len(broken_faces))

    if len(broken_faces) > 0:
        print("WARN fixmesh.py:Manifold output still has broken faces. Falling back to original mesh.")
        mesh = trimesh.load_mesh(mesh_filename)
        print("INFO fixmesh.py:Original mesh broken faces: %s", trimesh.repair.broken_faces(mesh))

    broken_faces = trimesh.repair.broken_faces(mesh)
    if len(broken_faces) > 0:
        print("WARN fixmesh.py:Proceeding despite %d broken faces." % len(broken_faces))

    if mesh.vertices.shape[0] > target_vertices and mesh.vertices.shape[0] < 10 * target_vertices:
        print("INFO fixmesh.py:Trying to reach target vertices with Garland quadric decimation (fast path).")
        ...
        if len(broken_faces) == 0 and np.abs(resampled_mesh.vertices.shape[0] - target_vertices) < tolerance_num_vertices:
            print("INFO fixmesh.py:Successfully reached target vertices with Garland decimation: vertices=%d" % resampled_mesh.vertices.shape[0])
            mesh = resampled_mesh
            mesh.export(output_filename)
            return mesh
        else:
            print("INFO fixmesh.py:Garland decimation did not reach target or produced broken faces (vertices=%d, broken=%d)." %
                        (resampled_mesh.vertices.shape[0], len(broken_faces)))
    ...
    if len(broken_faces) == 0 and within_range(mesh.vertices.shape[0]):
        print("INFO fixmesh.py:Mesh already valid and within target vertex range (%d)." % mesh.vertices.shape[0])
        mesh.export(output_filename)
        return mesh
    ...
    while not no_holes or attempts <= 20:
        print("INFO fixmesh.py:Simplifying mesh (target_faces=%d, aggressivity=%d, attempt=%d)" % (target_faces, aggressivity, attempts))
        if attempts > 5:
            print("INFO fixmesh.py:More than 5 attempts: applying additional Laplacian smoothing (iterations=%d)." % smooth_iteration)
            mesh = trimesh.smoothing.filter_laplacian(mesh, iterations=smooth_iteration)
            smooth_iteration += 5
            aggressivity = 5

        print("DEBUG fixmesh.py:Running trimesh.simplify_quadric_decimation with aggressivity=%d and additional faces=%d" % (aggressivity, more_faces))
        ...
        if resampled_mesh.vertices.shape[0] == target_vertices:
            print("INFO fixmesh.py:Resampled mesh matches target vertices exactly: %d" % target_vertices)
        else:
            print("INFO fixmesh.py:Resampled mesh vertices=%d (target=%d). Attempting alternative simplification if applicable." % (
                        resampled_mesh.vertices.shape[0], target_vertices))
            if resampled_mesh.vertices.shape[0] <= 5 * target_vertices:
                print("INFO fixmesh.py:Attempting secondary simplification with Garland quadric decimation (quad_mesh_simplify).")
                resampled_mesh = quadric_decimation_garland(resampled_mesh, target_vertices)
                print("INFO fixmesh.py:Vertices after Garland pass: %d; broken_faces=%d" % (
                            resampled_mesh.vertices.shape[0], len(trimesh.repair.broken_faces(resampled_mesh))))
            else:
                print("DEBUG fixmesh.py:Skipping Garland decimation because vertex count is far from target.")


            # save mesh as a.obj file
            broken_faces = trimesh.repair.broken_faces(resampled_mesh)
            print("INFO fixmesh.py:Broken faces after quadric decimation: %d" % len(broken_faces))
            if len(broken_faces) > 0:
                print("INFO fixmesh.py:Filling holes in resampled mesh.")
                # trimesh.repair.fill_holes(resampled_mesh)
                if use_mesh_fix:
                    print("INFO fixmesh.py:Applying mesh_fix (pymeshfix) to resampled mesh.")
                    ...
                    if not within_range(resampled_mesh.vertices.shape[0]) and len(broken_faces) == 0:
                        print("INFO fixmesh.py:Adding vertices by edge split to reach target after mesh_fix.")
                        resampled_mesh = add_vertices_by_edge_split(resampled_mesh, target_vertices)
                        print("INFO fixmesh.py:Vertex count after edge split: %d" % (resampled_mesh.vertices.shape[0],))
            ...
            if len(broken_faces) == 0 and within_range(resampled_mesh.vertices.shape[0]):
                ...
                print("INFO fixmesh.py:Retrying: current vertices=%d, broken_faces=%d, next aggressivity=%d" % (
                            resampled_mesh.vertices.shape[0], len(broken_faces), aggressivity))
            attempts += 1
            if attempts > 20:
                if not no_holes:
